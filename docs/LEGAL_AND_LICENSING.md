...
| whisper.cpp code + ggml weights (`base.en`, `small`, `large-v3-turbo`) | Pin release + model hash in Phase 5 | MIT (code); weights permissive per release — verify per model | Bundled/in-process (`whisper-rs`) | OK; record model card URL + hash | — | 🚧 strategy set, pins TBD |
