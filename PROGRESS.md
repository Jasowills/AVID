...
## Phase 3 — Timeline ✅ (engine + working UI; markers/slip pending)
...
- [x] Waveforms in timeline: peaks engine (decode + downsample, live tone/silence test) + session cache + `waveform_peaks` command + SVG rendering in audio clips
...
| 2026-09-21 | Identity pass | Monochrome-default theme engine (8 presets, runtime vars, picker, persistence), real LogoMark, slim chrome, density pass; Blue preserved as preset. Per MASTER-DESIGN. |
| 2026-09-22 | Match pass 2: Studio Light default, rail header, icon tabs | Default theme is now Studio Light (spec override: AGENTS §60 dark-first; monochrome preserved as preset). Rail header with project menu + view toggles, icon+label segmented tabs, panel icons. |
| 2026-09-22 | Match pass 1: asset library, director chat, Studio themes | Media asset grid (insert-at-playhead real), AI composer over the real pipeline (tested intent router), Studio Dark/Light presets (monochrome still default). |
| 2026-09-22 | Diffusion shell + ui kit port | Editor grid 1:1 (rails/dividers/Layers/resize/minimize/focus mode, persisted), command palette (⌘K), Tooltip/Kbd/ContextMenu kit, clip context menu, zoom/tools cell. MPL headers + `docs/DIFFUSION-PORT.md` + LEGAL row. |
| 2026-09-21 | Screen audit (all desktop screens) | Real-pixel audit via Playwright (`docs/SCREEN-AUDIT.md`): found invisible primary buttons (Tailwind v4 never scanned `packages/ui` — fixed with `@source`, guarded by `scripts/verify-web-build.sh`) + clipped Propose row (stacked). |
| 2026-09-21 | HARNESS audit + Director checklist | `docs/HARNESS-COMPARISON.md` (§§1–32 verdicts, top-5 deltas); AI panel Analyzed card (Transcript/Timeline-audio/Visuals/Assets, honest not-covered), numbered timecoded proposals with High/Medium confidence. |
| 2026-09-21 | Impeccable refinement pass | Stable-width buttons, object-specific loading copy, tokenized SVG canvas, themed selection/caret/focus, tabular numerals, structural skeletons, token-native toast system with success actions; mechanical detector clean on all touched surfaces. |
| 2026-09-21 | Release re-bundle | `AVID.app` rebuilt with harness UI (rail, dashboard, icons) and launched via `open` for click-through (dev shells don't survive headless). |
| 2026-09-21 | Harness-UI pass | Nav rail + dashboard Home (cards, provider/jobs status), working TopBar undo/redo + live AI pill, stale phase-copy removed, shared provider helper, Settings shell. |
| 2026-09-21 | Waveform pass | Peaks pipeline (engine + session cache + command + canvas rendering); 24 desktop tests. |
